...
//		echo "select $getfld from $tblName where $keyfld = '".$keyfldvalue."' ";
   		if(ets_db_num_rows($arrSql) > 0){
...
//	/* Product start */
	...
	function getproductType($productID){ 
		$customer_qry = ets_db_query("select * from producttype where product_type_id = '".$productID."' ") or die(ets_db_error());
		$rs = ets_db_fetch_array($customer_qry);
...
function getRealEstateTypeList()
{
    $sel_qry = "select * from real_estate_act_type where status = 'E' ";
    $sel_res = ets_db_query($sel_qry) or die(ets_db_error().$sel_qry);
    while($arr=ets_db_fetch_array($sel_res)){
        $type_arr[$arr['real_estate_act_type_id']] = $arr['real_estate_act_type_title'];
    }
    return $type_arr;
}
