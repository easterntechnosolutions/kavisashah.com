<?php
	...
	//$queryString =  ets_db_query("SELECT * from products where pTypeID = '".(int)$_REQUEST['pTypeID']."' order by sortorder") or die(ets_db_error());
		...
		while($res = ets_db_fetch_array($queryString)){
			...
			<a href="index.php?controller=products&action=productimages&subaction=listData&productID='.$res['productID'].'" title="Add Project Banner Image" data-toggle="tooltip" class="btn btn-info marker"><i class="fa fa-picture-o"></i></a> |
