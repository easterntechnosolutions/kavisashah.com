...
class banner
{
    function addForm()
    {
        ...
        $form->addElement(new Element_File("Banner Image: <br>(1920 × 550 pixels)", "image", array(
            "required" => 1,
            "placeholder" => "Photo"
        )));
    ...
    function delete()
    {
        $selqry = "Select * from banner_image where bannerID='".$_GET['bannerID']."'";
        $selres = ets_db_query($selqry);
        $arr = ets_db_fetch_array($selres);
        @unlink(DIR_FS_BANNER_PATH.$arr['image']);
        $delsql = "Delete from banner_image where bannerID='".$_GET['bannerID']."'";
