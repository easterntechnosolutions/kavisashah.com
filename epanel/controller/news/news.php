...
class news
{
	...
	/*	$form->addElement(new Element_Select("News/Event Type:", "news_type",$news, array(
		...
		$form->addElement(new Element_jQueryUIDate("Date:", "news_date", array(
        ...
        $form->addElement(new Element_File("News Image: <br>", "image", array(
            "required" => 1,
            "placeholder" => "Photo"
        )));
	...
	function add()
	{
        ...
        if(move_uploaded_file($_FILES["image"]["tmp_name"],DIR_FS_NEWS_PATH.$news_id.'-'.$target_path)){

            $updqry = "update news set 
			image = '".$news_id.'-'.$upload."'
