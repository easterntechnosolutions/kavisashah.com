...
class pages{
	...
	function editForm(){
		...
		if(ets_db_num_rows($qry) > 0){
            ...
            $form->addElement(new Element_Textarea("Mobile Page Content:", "mobile_page_content", array("value"=> stripslashes($rs['mobile_page_content']))
            ));
	...
	function edit(){		
		...
		meta_title = '".$pg_title."', 
		page_content = '".addslashes($_POST['page_content'])."',
		mobile_page_content = '".addslashes($_POST['mobile_page_content'])."',
		meta_keyword = '".$pgkeyword."',
		meta_desc = '".$pg_desc."',
		parent_id = '".$_POST['mainpages']."',
		sortorder = '".$_POST['sortorder']."',
...
<script>
	$('.table').editable({
		...
		value: 1,
		source: [{value: 'D', text: 'Draft'},{value: 'E', text: 'Published'}]
	});
</script>
<?php
	}
