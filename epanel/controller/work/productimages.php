...
class productimages
{
    function addForm()
    {
        ...
        $form->addElement(new Element_HTML("<legend>Brand's View</legend>"));
        ...
        $form->addElement(new Element_Hidden("isFront","B"));
       ...
       /* $form->addElement(new Element_File("Products Image: ", "master_plan", array(
        ...
        $form->addElement(new Element_File("Brand Grid Image : <br>(900 × 500 pixels)", "galleryImage", array(
            "placeholder" => "Brand Image Grid 1"
        )));
        
        $form->addElement(new Element_File("Brand Grid Image : <br>(900 × 500 pixels)", "galleryImage1", array(
            "placeholder" => "Brand Image Grid 2"
        )));
        
        $form->addElement(new Element_File("Brand Full Width Image (1920 × 700 pixels) : <br>", "galleryImage2", array(
            "placeholder" => "Brand Banner Image"
        )));
    ...
    function editForm()
    {
        ...
        if (ets_db_num_rows($qry) > 0) {
            ...
            $form->addElement(new Element_HTML("<legend>Edit Project View</legend>"));
            $form->addElement(new Element_Hidden("controller", "work"));
            $form->addElement(new Element_Hidden("action", "productimages"));
            ...
            $form->addElement(new Element_Textbox("View Title:", "galleryTitle", array(
                "required" => 1,
                "value" => stripcslashes($rs['galleryTitle'])
            )));
            

            if ($rs["galleryImage"] != "") {
                $thumbnail_path = DIR_WS_WORK_PATH . $_REQUEST['productID'] . '/' . $rs["galleryImage"];
                $form->addElement(new Element_HTML('<img src="'.$thumbnail_path.'" width="5%" height="10%" ">'));
                $form->addElement(new Element_Hidden("viewImage", $rs["galleryImage"]));
            }
            
            $form->addElement(new Element_File("Brand Image Grid 1 ", "galleryImage", array(
                "placeholder" => "Brand Image"
            )));
            
            if ($rs["galleryImage1"] != "") {
                $thumbnail_path1 = DIR_WS_WORK_PATH . $_REQUEST['productID'] . '/' . $rs["galleryImage1"];
                $form->addElement(new Element_HTML('<img src="'.$thumbnail_path1.'" width="5%" height="10%" ">'));
                $form->addElement(new Element_Hidden("viewImage", $rs["galleryImage1"]));
            }
           
            $form->addElement(new Element_File("Brand Image Grid 2 ", "galleryImage1", array(
                "placeholder" => "Brand Image"
            )));
            
             if ($rs["galleryImage2"] != "") {
                $thumbnail_path2 = DIR_WS_WORK_PATH . $_REQUEST['productID'] . '/' . $rs["galleryImage2"];
                $form->addElement(new Element_HTML('<img src="'.$thumbnail_path2.'" width="5%" height="10%" ">'));
                $form->addElement(new Element_Hidden("viewImage", $rs["galleryImage2"]));
            }
            $form->addElement(new Element_File("Brand Full Image ", "galleryImage2", array(
                "placeholder" => "Brand Image"
            )));
    ...
    function edit()
    {
        ...
        if (ets_db_query($sql)) {
            ...
            if ($_FILES['galleryImage1']['name'] != '') {
                ...
                move_uploaded_file($_FILES["galleryImage1"]["tmp_name"], $target_path1);
            }
            if ($_FILES['galleryImage2']['name'] != '') {
                $target_path2 = DIR_FS_WORK_PATH . $_REQUEST['productID'] . '/' . $tfilename2;
                @unlink(DIR_FS_WORK_PATH . $_REQUEST['pTypeID'] . '/' . $oldproImg2);
                move_uploaded_file($_FILES["galleryImage2"]["tmp_name"], $target_path2);
            }
    ...
    function listData()
    {
        ...
        <script>
            $(document).ready(function () {
                var listURL = "helperfunc/productimagesList.php?workID=<?php echo $_REQUEST['workID']; ?>";
	...
	<table class="table table-striped table-bordered dataTable" id="projectviewslist" style="width:100%;">
		...
		<tr>
			<th>Id</th>
			<th>Brand Image Grid 1</th>
			<th>Brand Image Grid 2</th>
			<th>Brand Full Image </th>
